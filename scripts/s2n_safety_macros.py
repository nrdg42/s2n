...
MACROS = {
    ...
    'PRECONDITION(condition)': dict(
        doc  = 'Ensures the `condition` is `true`, otherwise the function will `{bail}` with a `S2N_ERR_PRECONDITION_VIOLATION` error',
        impl = '__S2N_ENSURE_CONDITION((condition), {bail}(S2N_ERR_PRECONDITION_VIOLATION))',
        harness = '''
        static {ret} {prefix}PRECONDITION_harness(uint32_t v)
        {{
            {prefix}PRECONDITION(v != 1);
            return {ok};
        }}
        ''',
        tests = [
            '{expect_ok}({prefix}PRECONDITION_harness(0));',
            '{expect_err}({prefix}PRECONDITION_harness(1), S2N_ERR_PRECONDITION_VIOLATION);',
            '{expect_ok}({prefix}PRECONDITION_harness(2));',
        ],
    ),
    'POSTCONDITION(condition)': dict(
        doc  = 'Ensures the `condition` is `true`, otherwise the function will `{bail}` with a `S2N_ERR_POSTCONDITION_VIOLATION` error',
        impl = '__S2N_ENSURE_CONDITION((condition), {bail}(S2N_ERR_POSTCONDITION_VIOLATION))',
        harness = '''
        static {ret} {prefix}POSTCONDITION_harness(uint32_t v)
        {{
            {prefix}POSTCONDITION(v != 1);
            return {ok};
        }}
        ''',
        tests = [
            '{expect_ok}({prefix}POSTCONDITION_harness(0));',
            '{expect_err}({prefix}POSTCONDITION_harness(1), S2N_ERR_POSTCONDITION_VIOLATION);',
            '{expect_ok}({prefix}POSTCONDITION_harness(2));',
        ],
    ),
