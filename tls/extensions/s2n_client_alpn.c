...
static int s2n_client_alpn_recv(struct s2n_connection *conn, struct s2n_stuffer *extension)
{
    ...
    POSIX_GUARD(s2n_stuffer_init(&client_protos, &client_app_protocols));
    POSIX_GUARD(s2n_stuffer_write(&client_protos, &client_app_protocols));
    POSIX_GUARD(s2n_stuffer_init(&server_protos, server_app_protocols));
    POSIX_GUARD(s2n_stuffer_write(&server_protos, server_app_protocols));

    while (s2n_stuffer_data_available(&server_protos)) {
        uint8_t length;
        uint8_t server_protocol[255];
        POSIX_GUARD(s2n_stuffer_read_uint8(&server_protos, &length));
        POSIX_GUARD(s2n_stuffer_read_bytes(&server_protos, server_protocol, length));

        while (s2n_stuffer_data_available(&client_protos)) {
            uint8_t client_length;
            POSIX_GUARD(s2n_stuffer_read_uint8(&client_protos, &client_length));
            S2N_ERROR_IF(client_length > s2n_stuffer_data_available(&client_protos), S2N_ERR_BAD_MESSAGE);
            if (client_length != length) {
                POSIX_GUARD(s2n_stuffer_skip_read(&client_protos, client_length));
            } else {
                uint8_t client_protocol[255];
                POSIX_GUARD(s2n_stuffer_read_bytes(&client_protos, client_protocol, client_length));
                if (memcmp(client_protocol, server_protocol, client_length) == 0) {
                    POSIX_CHECKED_MEMCPY(conn->application_protocol, client_protocol, client_length);
        ...
        POSIX_GUARD(s2n_stuffer_reread(&client_protos));
    }
    return S2N_SUCCESS;
}
