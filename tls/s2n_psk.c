...
int s2n_psk_init(struct s2n_psk *psk, s2n_psk_type type)
{
    ...
    POSIX_CHECKED_MEMSET(psk, 0, sizeof(struct s2n_psk));
...
int s2n_psk_free(struct s2n_psk *psk)
{
    ...
    POSIX_GUARD(s2n_free(&psk->early_secret));
    POSIX_GUARD(s2n_free(&psk->identity));
    POSIX_GUARD(s2n_free(&psk->secret));
...
S2N_RESULT s2n_psk_parameters_wipe(struct s2n_psk_parameters *params)
{
    ...
    /* Free all PSKs */
    RESULT_GUARD(s2n_psk_parameters_free_unused_psks(params));
    RESULT_GUARD_POSIX(s2n_psk_free(params->chosen_psk));
...
int s2n_psk_parameters_free(struct s2n_psk_parameters *params)
{
    ...
    POSIX_GUARD_RESULT(s2n_psk_parameters_wipe(params));
    POSIX_GUARD(s2n_free(&params->psk_list.mem));
    return S2N_SUCCESS;
}
...
static int s2n_tls13_keys_init_with_psk(struct s2n_tls13_keys *keys, struct s2n_psk *psk)
{
    ...
    POSIX_GUARD(s2n_hash_hmac_alg(keys->hash_algorithm, &keys->hmac_algorithm));
        ...
        struct s2n_blob *output_binder)
{
    ...
    POSIX_GUARD(s2n_tls13_keys_init_with_psk(&psk_keys, psk));
        ...
        const struct s2n_blob *partial_client_hello, struct s2n_blob *binder_to_verify)
{
    ...
    POSIX_GUARD(s2n_psk_calculate_binder_hash(conn, psk->hash_alg, partial_client_hello, &binder_hash));
        ...
        struct s2n_stuffer *out)
{
    ...
    for (size_t i = 0; i < psk_list->len; i++) {
        ...
        if (binder_hash->size == 0) {
            uint8_t hash_size = 0;
            RESULT_GUARD_POSIX(s2n_hash_digest_size(psk->hash_alg, &hash_size));
            RESULT_GUARD_POSIX(s2n_blob_init(binder_hash, binder_hashes_data[psk->hash_alg], hash_size));
            RESULT_GUARD_POSIX(s2n_psk_calculate_binder_hash(conn, psk->hash_alg, partial_client_hello, binder_hash));
        }
