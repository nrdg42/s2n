...
{
    POSIX_ENSURE_REF(dh_params);
    POSIX_PRECONDITION(s2n_blob_is_valid(pkcs3));
                               ...
                               struct s2n_blob *Ys)
{
    ...
    POSIX_PRECONDITION(s2n_blob_is_valid(p));
    POSIX_PRECONDITION(s2n_blob_is_valid(g));
    POSIX_PRECONDITION(s2n_blob_is_valid(Ys));
...
{
    POSIX_GUARD(s2n_check_all_dh_params(server_dh_params));
    POSIX_PRECONDITION(s2n_stuffer_is_valid(out));
    POSIX_PRECONDITION(s2n_blob_is_valid(output));
