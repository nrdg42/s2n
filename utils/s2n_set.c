...
S2N_RESULT s2n_set_add(struct s2n_set *set, void *element)
{
    ...
    uint32_t index = 0;
    RESULT_GUARD(s2n_set_binary_search(set, element, &index));
    RESULT_GUARD(s2n_array_insert_and_copy(set->data, index, element));
...
S2N_RESULT s2n_set_get(struct s2n_set *set, uint32_t index, void **element)
{
    ...
    RESULT_GUARD(s2n_array_get(set->data, index, element));
...
{
    RESULT_GUARD(s2n_set_validate(set));
    RESULT_GUARD(s2n_array_remove(set->data, index));
